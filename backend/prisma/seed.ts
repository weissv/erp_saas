...
// Generic "Acme Corp" seed data for provisioning new tenants.
// No real-world personally-identifiable information.
...
// ---------------------------------------------------------------------------
// Generic student fixtures for grade-4 class
// ---------------------------------------------------------------------------
const studentsData = [
...
// ---------------------------------------------------------------------------
// Generic schedule (Mon–Fri, 5 lessons per day)
// ---------------------------------------------------------------------------
const scheduleItems = [
...
async function main() {
  ...
  for (const s of studentsData) {
    ...
    const existingChild = await prisma.child.findFirst({
      where: { firstName: s.firstName, lastName: s.lastName, groupId: demoClass.id },
    });

    if (existingChild) {
      const existingLms = await prisma.lmsSchoolStudent.findFirst({
        where: { studentId: existingChild.id, classId: demoClass.id },
      });
      if (!existingLms) {
        await prisma.lmsSchoolStudent.create({
          data: { studentId: existingChild.id, classId: demoClass.id, enrollmentDate: new Date(), status: "active" },
        });
      }
      continue;
    }
    ...
    const child = await prisma.child.create({
      data: {
        ...
        groupId: demoClass.id,
        status: "ACTIVE",
      },
    });

    await prisma.lmsSchoolStudent.create({
      data: { studentId: child.id, classId: demoClass.id, enrollmentDate: new Date(), status: "active" },
    });
  }
